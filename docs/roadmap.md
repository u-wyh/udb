...
- 85. Durable PageLSN Foundation：为每个数据页建立持久化 `pageLSN`；保持逻辑 `Page`/SlottedPage/B+Tree 格式不被强制重写，可使用安全的 sidecar 或等价设计；补齐真正的 WAL-before-data durability / fsync 顺序。
- 86. WAL Transaction Chains：每条事务日志增加 `prevLSN`；Transaction 维护 `lastLSN`；新增 CLR（Compensation Log Record）和 `undoNextLSN`，并版本化 WAL 格式。
...
- 88. ARIES Redo Pass：从最小 `recLSN` 开始 repeat history；根据 DPT 与持久化 pageLSN 判断是否真正重做 PageWrite / Allocate / Free；redo 必须幂等。
...
- 92. WAL Truncation：根据 checkpoint、DPT `recLSN` 和活跃事务安全确定最早必需日志；只回收确定不再需要的 WAL，保证 restart recovery 正确。
